...
public class ConsoleApplication {
	...
	//Creating an int that is equal to the length of the options array e.g 8 
	...
	//Creating an instance of MP3Player
		...
		do {
			...
			if (choice != QUIT) {
				...
				case 5:
					...
					break;
				case 6:
					// Switch on method to turn on the MP3Player
					switchOn();
					break;
				case 7: 
					// Switch off method to turn off the MP3Player
					switchOff();
					break;
				default: 
	...
	private static void addSomeTunes(){
		...
		mp3Player.addtune("My Patch", "Jim Noir", 400, Genre.POP);
		mp3Player.addtune("Key of C", "Jim Noir", 400, Genre.POP);
		mp3Player.addtune("All Blues", "Miles Davis", 2000, Genre.JAZZ);
		mp3Player.addtune("Around the World", "Daft Punk", 1500, Genre.DANCE);
		mp3Player.addtune("Song 2", "DJ Krush", 750, Genre.OTHER);
		mp3Player.addtune("Harvest Dawn", "Jeremy Soule", 350, Genre.CLASSICAL);
		mp3Player.addtune("Do you realize?", "The Flaming Lips", 300, Genre.ROCK);
	...
	private static void selectFromFullList() {
		if (isOn) {
			String[] tuneInfo = mp3Player.getTuneInfo();
			...
			System.out.println(mp3Player.play(Integer.parseInt(splitID[0])));
	...
	private static void selectTuneByArtist() {
		if (isOn) {
			...
			String[] songs = mp3Player.getTuneInfo();
			...
			String[] artistTunes = mp3Player.getTuneInfo(chosenArtist);
			...
			//			System.out.println(splitID[0] + "TEST");
			boolean ok = false;
			do {
				try {
	...
	private static void selectFromGenre() {
		if (isOn) {
			...
			String[] genreTunes = mp3Player.getTuneInfo(option2);
	...
	// A method that uses a boolean to turn on the MP3Player
	...
	// This method uses boolean to turn the MP3Player off
	...
	private static void addATune() {
		if (isOn == true) {
			...
			mp3Player.addtune(title, artist, duration, gen);
